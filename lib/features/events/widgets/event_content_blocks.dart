...
/// Renders the populated `contentBlocks` of an Event-Entry. Falls back to
...
class EventContentBlocks extends StatelessWidget {
  final List<EventContentBlock> blocks;
  final String fallbackText;
  final String heroTagPrefix;

  const EventContentBlocks({
    super.key,
  ...
  Widget _buildBlock(BuildContext context, EventContentBlock block, int index) {
    switch (block) {
      case EventTextBlock(body: final body):
        if (body.trim().isEmpty) return const SizedBox.shrink();
        return Padding(
          padding: const EdgeInsets.all(8),
          child: MarkdownBlock(data: body, config: getMarkdownConfig(context)),
        );
      case EventMediaBlock(media: final media):
        if (media == null) return const SizedBox.shrink();
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 8),
          child: media.isVideo
              ? _EventBlockVideo(url: media.url, semanticLabel: media.name)
              : _EventBlockImage(
                  url: media.url,
                  width: media.width,
                  height: media.height,
...
class _EventBlockImage extends StatelessWidget {
  final String url;
  final int? width;
  final int? height;
  ...
  Widget build(BuildContext context) {
    final ratio = (width != null && height != null && height! > 0)
        ? width! / height!
        : 16 / 9;
    final label = semanticLabel.isNotEmpty ? semanticLabel : 'Bild zum Event';
...
class _EventBlockVideo extends StatefulWidget {
  final String url;
  final String semanticLabel;

  const _EventBlockVideo({required this.url, required this.semanticLabel});

  @override
  State<_EventBlockVideo> createState() => _EventBlockVideoState();
}

class _EventBlockVideoState extends State<_EventBlockVideo> {
